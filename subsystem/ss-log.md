...
## 2026-06-21 · Onboarding fixes · Portable backlog database · Seeding from embedded requests.json
Task 0 is completed and verified. Backlog database restructured to enforce "one truth, batteries included" doctrine:
- **Onboard.cs fixes**: Resolved CLAUDE.md worktree redirection; generalized header matching (supports both new and old Markdown section styles). Run space orientation is now fully functional on a clean checkout.
- **Global ss alias**: Added C:\bin\ss.cmd to route commands to S:\ss.exe, making it globally accessible on PATH.
- **Workspace rules**: Added .agents/AGENTS.md to automate agent onboarding and direct cold launches to `ss onboard`.
- **Integrated requests backlog**: Reverted AppData repoint (RequestHive is back to subsystem-requests.db but located locally beside the exe by default). Created git-versioned `src/runspace/Cm/requests.json` and compiled it into `ss.exe`. Added automatic first-boot database seeding from embedded resource.
- **Double-gated release wipe**: Gated the `--clear-for-release` / `-c` flag in Build.cs and SelfBuild.cs with interactive confirmations (y/N and yes/NO) to protect the DB from accidental wipe.
- **Backlog seeded**: Verified `ss Get-Request` dynamically seeds and prints INC000000000103, CRQ000000000001, and CRQ000000000002.
- **Reflection Cleanup**: Completed Vom owner reflection cleanup; Tui.cs reads GetOwners() strongly typed.
